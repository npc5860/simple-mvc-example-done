...
const hostPage4 = async (req, res) => {
  try {
    const entries = await Dog.find({}).lean().exec();

    return res.render('page4', { dogs: entries });
  } catch (e) {
    console.log(e);
...
const ageDog = async (req, res) => {
  ...
  try {
    const entry = await Dog.findOne({ name: req.body.name }).exec();
    ...
    return res.json({ name: entry.name, age: entry.age });
  } catch (e) {
    console.log(e);
